...
class generateMetapeaks(Scene):
    ...
    def construct(self):
        ax = Axes(
            x_range=[900, 920, 5],
            y_range=[0, 20000, 4000], # incorrect values, but they get hidden later 
            #y_range=[0, 1, 0.2], # incorrect values, but they get hidden later 
            ...
            y_axis_config={
                "color": WHITE,
                "include_numbers": True  # Hiding the y-axis numbers as well
            },
            axis_config={"color": WHITE}
        )
        
        # Add labels for x and y axis
        #x_label = Text("m/z").next_to(ax.x_axis, DOWN)

        #self.add(ax, x_label)

        # Initialize sigma and mu(distribution standard deviation) ValueTrackers
        mu = ValueTracker(910)
        sigma = ValueTracker(1)

        # text for mu and sigma symbols
        ...
        everything_group = VGroup(ax, curve_to_fill, curve_fill, lines)  # Include all other objects that are part of the scene
